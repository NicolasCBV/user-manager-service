...
export const createRelaunchOTPE2E = async ({
  shouldCreateContent 
    ...
    const otp = OTPFactory({
      ...
      createdAt: shouldCreateContent?.time ?? new Date(
        Date.now() - parseInt(process.env.OTP_TIME ?? '120000') + 10000
      )
    });
    ...
    if(shouldCreateContent) {
      ...
      await miscHandler.startUserSigin(
        user,
        otp, 
        cancelKey
      );
